/* ============================================================
 deck-layouts.js v6.0.6 -- Layout Shortcut Library
 ...
 v6.0.3:  coverPresenter v3 — brand template, asset prefetch
 v6.0.4:  coverPresenter v4 — title L2, subtitle L3, spacing
 v6.0.5:  section layout — asymmetric dark divider
 v6.0.6:  textContent layout — standard content with
          light/dark variants, engine-managed content footer
 ============================================================ */
...
if (cfg.tag) {
  els.push({ type: 't', text: cfg.tag, x: C.SAFE_X_MIN, y: C.TAG_Y, w: 11.00, h: C.TAG_H, font: 'H', size: 11, color: 'accent' });
}
var titleY = 2.00;
var titleH = titleLen > 20 ? 1.80 : 1.20;
els.push({ type: 't', text: cfg.title, x: C.SAFE_X_MIN, y: titleY, w: 11.00, h: titleH, font: 'H', size: 42, color: 'title' });
var subY = titleY + titleH + 0.15;
if (cfg.subtitle) {
  els.push({ type: 't', text: cfg.subtitle, x: C.SAFE_X_MIN, y: subY, w: 11.00, h: 0.40, font: 'H', size: 22, color: 'sub' });
}
var dateY = subY + (cfg.subtitle ? 0.50 : 0);
if (cfg.date) {
  els.push({ type: 't', text: cfg.date, x: C.SAFE_X_MIN, y: dateY, w: 11.00, h: 0.30, font: 'B', size: 13, color: 'body' });
}
return els;
}
...
if (cfg.attribution) {
  els.push({ type: 't', text: cfg.attribution, x: C.SAFE_X_MIN, y: subY + 0.55, w: 11.00, h: 0.30, font: 'B', size: 11, color: 'body' });
}
return els;
}
...
var titleH = 1.10;
var els = [{ type: 't', text: cfg.title, x: C.SAFE_X_MIN, y: centerY, w: 11.00, h: titleH, font: 'H', size: 42, color: 'title', valign: 'middle' }];
if (cfg.subtitle) {
  els.push({ type: 't', text: cfg.subtitle, x: C.SAFE_X_MIN, y: centerY + titleH + 0.20, w: 11.00, h: 0.40, font: 'B', size: 18, color: 'sub' });
}
return els;
}
...
items.forEach(function(item, i) {
  ...
  var innerY = cy + 0.20;
  if (item.icon) { els.push({ type: 'i', icon: item.icon, x: textX, y: innerY, w: 0.50, h: 0.50 }); innerY += 0.60; }
  if (item.title) { els.push({ type: 't', text: item.title, x: textX, y: innerY, w: textW, h: 0.45, font: 'H', size: 15, color: 'title' }); innerY += 0.55; }
  if (item.text) { els.push({ type: 't', text: item.text, x: textX, y: innerY, w: textW, h: cardH - (innerY - cy) - 0.60, font: 'B', size: 13, color: 'body' }); }
  if (item.pill) { els.push({ type: 'p', text: item.pill, x: textX, y: cy + cardH - 0.45, w: 1.50, h: 0.30, fill: item.pillColor || 'accent', color: '#FFFFFF', size: 9 }); }
});
return els;
}
...
items.forEach(function(item, i) {
  ...
  if (item.text) { els.push({ type: 't', text: item.text, x: textX, y: cy + 1.10, w: textW, h: cellH - 1.35, font: 'B', size: 11, color: 'body' }); }
});
return els;
}
...
var maxValueLen = 0;
items.forEach(function(item) { if (item.value && item.value.length > maxValueLen) maxValueLen = item.value.length; });
var valueSize = 44;
var estimatedWidth = maxValueLen * 0.40;
var availableWidth = sampleTextW * 0.85;
if (estimatedWidth > availableWidth) { valueSize = Math.max(28, Math.floor(44 * (availableWidth / estimatedWidth))); }
items.forEach(function(item, i) {
  var col = i % cols;
  var row = Math.floor(i / cols);
  var cx = grid.cols[col].x;
...
items.forEach(function(item, i) {
  var iy = cardY + 0.20 + i * 0.60;
  if (item.icon) { els.push({ type: 'i', icon: item.icon, x: innerX, y: iy, w: 0.40, h: 0.40 }); }
  ...
  if (i < items.length - 1) { els.push({ type: 'd', x: innerX, y: iy + 0.48, w: textW, color: 'ltGray' }); }
});
return els;
}
...
items.forEach(function(item, i) {
  var by = startY + i * 0.55;
  els.push({ type: 'o', x: C.SAFE_X_MIN + 0.10, y: by + 0.18, w: 0.12, h: 0.12, fill: 'accent' });
  els.push({ type: 't', text: item, x: bulletX, y: by, w: 10.00, h: 0.55, font: 'B', size: 15, color: 'body', valign: 'middle' });
});
return els;
}
...
items.forEach(function(item, i) {
  ...
  if (item.subtitle) { els.push({ type: 't', text: item.subtitle, x: textX, y: startY + 1.30, w: textW, h: 0.25, font: 'H', size: 11, color: 'muted' }); }
...
var rtw = rw * C.TEXT_RATIO; var rtx = rx + (rw - rtw) / 2;
...
if (cfg.from) {
  ...
  if (cfg.from.label) { els.push({ type: 't', text: cfg.from.label, x: rtx, y: startY + 1.15, w: rtw, h: 0.30, font: 'B', size: 11, color: 'body' }); }
}
...
if (cfg.to) {
  ...
  if (cfg.to.label) { els.push({ type: 't', text: cfg.to.label, x: rtx, y: toY + 1.15, w: rtw, h: 0.30, font: 'B', size: 11, color: 'white' }); }
}
var lx = grid.cols[0].x; var lw = grid.cols[0].w;
var ltw = lw * C.TEXT_RATIO; var ltx = lx + (lw - ltw) / 2;
if (cfg.description) { els.push({ type: 't', text: cfg.description, x: ltx, y: startY + 0.20, w: ltw, h: availH * 0.45, font: 'B', size: 13, color: 'body' }); }
if (cfg.benefits) {
  var benefitsText = cfg.benefits;
  if (Array.isArray(cfg.benefits)) { benefitsText = cfg.benefits.map(function(b) { return typeof b === 'object' ? (b.text || b) : b; }).join(' | '); }
  els.push({ type: 's', x: lx, y: toY, w: lw, h: toBlockH, fill: isDark ? 'dkGray' : 'white', border: 'accent' });
  els.push({ type: 't', text: 'BENEFITS', x: ltx, y: toY + 0.15, w: ltw, h: 0.20, font: 'H', size: 9, color: 'accent' });
  els.push({ type: 't', text: benefitsText, x: ltx, y: toY + 0.45, w: ltw, h: toBlockH - 0.65, font: 'B', size: 12, color: 'accent', valign: 'top' });
}
return els;
}
...
var labelH = 0.25; var cellPad = 0.05;
...
columns.forEach(function(colName, ci) {
  ...
  var cx = grid.cols[ci].x; var cw = grid.cols[ci].w;
...
items.forEach(function(row, ri) {
  ...
  if (row.metric) { els.push({ type: 't', text: row.metric, x: C.SAFE_X_MIN, y: ry, w: 3.00, h: labelH, font: 'H', size: 10, color: isDark ? 'accentLt' : 'accent' }); }
  var cellY = ry + labelH + cellPad;
  var values = row.values || [];
  values.forEach(function(val, vi) {
    if (vi >= colCount || !grid.cols[vi]) return;
    var cx = grid.cols[vi].x; var cw = grid.cols[vi].w;
...
var items = cfg.items || [];
var availH = C.CONTENT_END - startY;
var headerH = 0.40; var headerGap = C.GAP;
...
var timeX = C.SAFE_X_MIN; var timeW = 2.50;
var actX = timeX + timeW + C.GAP; var actW = 6.50;
var whoX = actX + actW + C.GAP; var whoW = C.SAFE_W - timeW - actW - C.GAP * 2;
...
if (cfg.location) { els.push({ type: 't', text: cfg.location, x: C.SAFE_X_MIN, y: locY, w: 11.00, h: 0.45, font: 'H', size: 24, color: 'sub' }); }
if (cfg.date) { els.push({ type: 't', text: cfg.date, x: C.SAFE_X_MIN, y: locY + 0.60, w: 11.00, h: 0.35, font: 'B', size: 16, color: 'body' }); }
return els;
}
...
// ============================================================
// CLIENT LAYOUT: COVER-PRESENTER v4
// ============================================================
...
var grid = getGrid(1);
var cx = grid.cols[0].x; var cw = grid.cols[0].w; var textW = cw * 0.65;
var GOLD = '#CAA380';
els.push({ type: 'img', src: CP_LOGO_URL, x: 11.29, y: 0.84, w: 1.02, h: 0.84 });
var titleY = 1.60;
if (cfg.title) { els.push({ type: 't', text: cfg.title, x: cx, y: titleY, w: textW, h: 0.80, font: 'B', size: 40, color: 'title', textStyle: 'L2' }); }
var subY = titleY + 1.05;
if (cfg.subtitle) { els.push({ type: 't', text: cfg.subtitle, x: cx, y: subY, w: textW, h: 0.45, font: 'B', size: 23, color: GOLD, textStyle: 'L3' }); }
var dateY = subY + 0.50;
if (cfg.date) { els.push({ type: 't', text: cfg.date, x: cx, y: dateY, w: textW, h: 0.35, font: 'B', size: 18, color: GOLD, textStyle: 'L3' }); }
var items = cfg.items || [];
var presStartY = dateY + 1.30;
items.forEach(function(item, i) {
  var rowY = presStartY + (i * 0.75);
  if (rowY + 0.64 <= 6.45) {
    els.push({ type: 't', text: item.title, x: cx, y: rowY, w: textW, h: 0.32, font: 'H', size: 18, color: 'white', textStyle: 'L3' });
    if (item.text) { els.push({ type: 't', text: item.text, x: cx, y: rowY + 0.32, w: textW, h: 0.32, font: 'B', size: 18, color: 'white', textStyle: 'L4' }); }
  }
});
els.push({ type: 'd', x: cx, y: 6.55, w: cw, color: 'white' });
if (cfg.footerOrg) { els.push({ type: 't', text: cfg.footerOrg, x: cx, y: 6.65, w: 6.00, h: 0.18, font: 'H', size: 8, color: 'white', textStyle: 'L3' }); }
if (cfg.footerDept) { els.push({ type: 't', text: cfg.footerDept, x: cx, y: 6.83, w: 6.00, h: 0.18, font: 'H', size: 8, color: 'white', textStyle: 'L3' }); }
return els;
}

// ============================================================
// CLIENT LAYOUT: SECTION — asymmetric dark divider
// ============================================================
...
var textColor = 'white';
if (cfg.num) { els.push({ type: 't', text: cfg.num, x: 0.89, y: 3.34, w: 0.91, h: 0.42, font: 'H', size: 20, color: textColor, valign: 'top' }); }
if (cfg.title) { els.push({ type: 't', text: cfg.title, x: 8.60, y: 3.34, w: 3.86, h: 0.42, font: 'H', size: 20, color: textColor, valign: 'top' }); }
var footerY = 6.40;
els.push({ type: 'd', x: C.SAFE_X_MIN, y: footerY, w: C.SAFE_W, color: textColor });
var orgText = cfg.footerOrg || 'Mazda North American Operations';
els.push({ type: 't', text: orgText, x: C.SAFE_X_MIN, y: footerY + 0.10, w: 8.00, h: 0.15, font: 'H', size: 10, color: textColor, textStyle: 'L5' });
var confText = cfg.footerConf || 'Confidential';
els.push({ type: 't', text: confText, x: C.SAFE_X_MIN, y: footerY + 0.25, w: 8.00, h: 0.15, font: 'B', size: 10, color: textColor });
return els;
}

// ============================================================
// CLIENT LAYOUT: TEXTCONTENT — standard content slide [v6.0.6]
// Light: white bg, black text. Dark: branded gradient,
// white text, #C4A484 subtitle. Engine handles content footer.
// ============================================================

var TC_DARK_SUB = '#C4A484';

function layoutTextContent(cfg) {
var els = [];
var isDark = cfg.dark === 1;

// Dark variant uses branded gradient (same as coverPresenter)
if (isDark) {
  cfg.bgGradient = 'url(' + CP_BG_URL + ') center/cover no-repeat, ' + CP_GRADIENT;
  cfg.bgImage = CP_BG_URL;
}
...
var subColor = isDark ? TC_DARK_SUB : '#000000';

if (cfg.title) {
  els.push({
    type: 't', text: cfg.title,
    x: 0.89, y: 0.75, w: 11.00, h: 0.55,
    font: 'H', size: 24, color: textColor,
    textStyle: 'L2'
  });
}
...
if (cfg.subtitle) {
  els.push({
    ...
    x: 0.89, y: 3.00, w: 6.00, h: 0.30,
    font: 'H', size: 14, color: subColor,
    textStyle: 'L3'
  });
}
...
if (cfg.text) {
  els.push({
    type: 't', text: cfg.text,
    x: 0.89, y: 3.50, w: 6.00, h: 3.00,
    font: 'B', size: 12, color: textColor,
    textStyle: 'L4'
  });
}

// No footer elements — engine content footer handles it
return els;
}
