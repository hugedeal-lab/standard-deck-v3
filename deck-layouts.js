...
// ============================================================
// SHARED: Timeline bar renderer with gap enforcement
// ============================================================

function _tlTrack(els, bars, tcY, tlX, tlW, numP, lblSize, lblMinW) {
var DR = 0.055; var LH = 0.018; var MIN_GAP = DR * 2 + 0.04;
...
disp.forEach(function(dp) {
  var bar = dp.bar; var bc = (bar.accent || bar.color === 'gold') ? 'accentLt' : 'title';
...
var els = []; var isDark = cfg.dark === 1;
var LBL_W = 2.20; var TL_X = C.SAFE_X_MIN + LBL_W + 0.12; var TL_W = C.SAFE_X_MAX - TL_X;
var HDR_Y = 1.25; var HDR_H = 0.55; var ROWS_Y = HDR_Y + HDR_H + 0.12;
var AVAIL_H = C.CONTENT_END - ROWS_Y; var PAD = 0.20; var GAP = 0.12;
var periods = cfg.periods || []; var numP = Math.max(periods.length, 1);
var rows = cfg.rows || []; var colW = TL_W / numP;

if (cfg.title) {
  els.push({ type:'t', text:cfg.title, x:C.SAFE_X_MIN, y:C.TAG_Y, w:11.00, h:0.40, font:'H', size:30, color:'title' });
}
els.push({ type:'s', x:C.SAFE_X_MIN, y:HDR_Y, w:C.SAFE_W, h:HDR_H, fill:'dkGray' });
els.push({ type:'t', text:cfg.headerLabel || 'CALENDAR WEEKS', x:C.SAFE_X_MIN + 0.15, y:HDR_Y, w:LBL_W - 0.15, h:HDR_H, font:'H', size:10, color:'accentLt', valign:'middle' });
...
periods.forEach(function(wk, i) {
  ...
  els.push({ type:'t', text:String(wk), x:cx-R, y:cy-R, w:R*2, h:R*2, font:'H', size:13, color:'title', align:'center', valign:'middle' });
});

periods.forEach(function(wk, i) {
  if (i === 0) return;
  els.push({ type:'s', x:TL_X + i*colW - 0.005, y:ROWS_Y, w:0.010, h:AVAIL_H, fill:'mdGray', transparency:78 });
});

var totalTracks = rows.reduce(function(s, r) { return s + (Array.isArray(r.tracks) ? r.tracks.length : 1); }, 0);
var trackH = Math.max(0.35, Math.min(0.52, (AVAIL_H - rows.length*PAD - (rows.length-1)*GAP) / Math.max(totalTracks, 1)));
...
rows.forEach(function(row, ri) {
  ...
  els.push({ type:'t', text:row.label || '', x:C.SAFE_X_MIN + 0.10, y:curY, w:LBL_W - 0.10, h:rowH, font: isAccent ? 'H' : 'B', size:12, color: isAccent ? 'accentLt' : 'body', valign:'middle' });
...
var els = []; var isDark = cfg.dark === 1;
var LBL_W = 2.20; var TL_X = C.SAFE_X_MIN + LBL_W + 0.12; var TL_W = C.SAFE_X_MAX - TL_X;
var HDR_Y = 1.25; var HDR_H = 0.55; var ROWS_Y = HDR_Y + HDR_H + 0.10;
var AVAIL_H = C.CONTENT_END - ROWS_Y; var PAD = 0.12; var GAP = 0.08;
var periods = cfg.periods || []; var numP = Math.max(periods.length, 1);
...
els.push({ type:'t', text:cfg.headerLabel || 'QUARTERS', x:C.SAFE_X_MIN + 0.15, y:HDR_Y, w:LBL_W - 0.15, h:HDR_H, font:'H', size:10, color:'white', valign:'middle' });

periods.forEach(function(qtr, i) {
  var px = TL_X + i*colW;
  els.push({ type:'s', x:px, y:HDR_Y, w:colW, h:HDR_H, fill:'accent' });
  els.push({ type:'t', text:String(qtr), x:px + 0.10, y:HDR_Y, w:colW - 0.20, h:HDR_H, font:'H', size:12, color:'white', align:'center', valign:'middle' });
});

// Crosshatch layer 1: alternating column fills
periods.forEach(function(qtr, i) {
  if (i % 2 !== 0) return;
  els.push({ type:'s', x:TL_X + i*colW, y:ROWS_Y, w:colW, h:AVAIL_H, fill:'dkGray', transparency:40 });
});
...
if (subDivs > 1) {
  for (var pi = 0; pi < numP; pi++) {
    for (var si = 1; si < subDivs; si++) {
      ...
      els.push({ type:'s', x:sgx - 0.004, y:ROWS_Y, w:0.008, h:AVAIL_H, fill:'mdGray', transparency:85 });
    }
  }
}
...
periods.forEach(function(qtr, i) {
  if (i === 0) return;
  els.push({ type:'s', x:TL_X + i*colW - 0.006, y:ROWS_Y, w:0.012, h:AVAIL_H, fill:'mdGray', transparency:65 });
});

var totalTracks = rows.reduce(function(s, r) { return s + (Array.isArray(r.tracks) ? r.tracks.length : 1); }, 0);
var trackH = Math.max(0.28, Math.min(0.48, (AVAIL_H - rows.length*PAD - (rows.length-1)*GAP) / Math.max(totalTracks, 1)));
...
rows.forEach(function(row, ri) {
  ...
  // Crosshatch layer 2: alternating row tints
  if (ri % 2 === 1) {
    els.push({ type:'s', x:C.SAFE_X_MIN, y:curY, w:C.SAFE_W, h:rowH, fill:'dkGray', transparency:50 });
  }

  els.push({ type:'t', text:row.label || '', x:C.SAFE_X_MIN + 0.10, y:curY, w:LBL_W - 0.10, h:rowH, font: isAccent ? 'H' : 'B', size:11, color: isAccent ? 'accentLt' : 'title', valign:'middle' });
