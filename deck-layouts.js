/* ============================================================
 deck-layouts.js v6.0.7 -- Layout Shortcut Library
 v6.0.5:  section layout — asymmetric dark divider
 v6.0.6:  textContent layout (replaced in v6.0.7)
 v6.0.7:  prose layout replaces textContent — evaluator-precise
          positioning, renderHeader, standard grid, dynamic Y
 ============================================================ */
...
registerPrefetch(CP_LOGO_URL); registerPrefetch(CP_BG_URL);

// ============================================================
// CLIENT: COVER-PRESENTER v4
// ============================================================

function layoutCoverPresenter(cfg) {
  var els=[]; cfg.customFooter=true;
  cfg.bgGradient='url('+CP_BG_URL+') center/cover no-repeat, '+CP_GRADIENT;
  cfg.bgImage=CP_BG_URL;
  var grid=getGrid(1); var cx=grid.cols[0].x; var cw=grid.cols[0].w; var tw=cw*0.65;
  var GOLD='#CAA380';
  els.push({type:'img',src:CP_LOGO_URL,x:11.29,y:0.84,w:1.02,h:0.84});
  var tY=1.60;
  if(cfg.title) els.push({type:'t',text:cfg.title,x:cx,y:tY,w:tw,h:0.80,font:'B',size:40,color:'title',textStyle:'L2'});
  var sY=tY+1.05;
  if(cfg.subtitle) els.push({type:'t',text:cfg.subtitle,x:cx,y:sY,w:tw,h:0.45,font:'B',size:23,color:GOLD,textStyle:'L3'});
  var dY=sY+0.50;
  if(cfg.date) els.push({type:'t',text:cfg.date,x:cx,y:dY,w:tw,h:0.35,font:'B',size:18,color:GOLD,textStyle:'L3'});
  (cfg.items||[]).forEach(function(item,i){
    var ry=dY+1.30+(i*0.75);
    if(ry+0.64<=6.45){
      els.push({type:'t',text:item.title,x:cx,y:ry,w:tw,h:0.32,font:'H',size:18,color:'white',textStyle:'L3'});
      if(item.text) els.push({type:'t',text:item.text,x:cx,y:ry+0.32,w:tw,h:0.32,font:'B',size:18,color:'white',textStyle:'L4'});
    }
  });
...
function layoutSection(cfg) {
  var els=[]; cfg.customFooter=true; cfg.dark=1; cfg.bgGradient=SECTION_BG;
  var tc='white';
  if(cfg.num) els.push({type:'t',text:cfg.num,x:0.89,y:3.34,w:0.91,h:0.42,font:'H',size:20,color:tc,valign:'top'});
  if(cfg.title) els.push({type:'t',text:cfg.title,x:8.60,y:3.34,w:3.86,h:0.42,font:'H',size:20,color:tc,valign:'top'});
  var fY=6.40;
  els.push({type:'d',x:C.SAFE_X_MIN,y:fY,w:C.SAFE_W,color:tc});
  els.push({type:'t',text:cfg.footerOrg||'Mazda North American Operations',x:C.SAFE_X_MIN,y:fY+0.10,w:8.00,h:0.15,font:'H',size:10,color:tc,textStyle:'L5'});
  els.push({type:'t',text:cfg.footerConf||'Confidential',x:C.SAFE_X_MIN,y:fY+0.25,w:8.00,h:0.15,font:'B',size:10,color:tc});
  return els;
}

// ============================================================
// CLIENT: PROSE — basic narrative slide [v6.0.7]
/* ============================================================
 deck-layouts.js v6.0.8 -- Layout Shortcut Library
 v6.0.5: section layout
 v6.0.6: textContent (replaced by prose in v6.0.7)
 v6.0.7: prose — evaluator-precise narrative layout
 v6.0.8: twoCols — 2-column image + text spotlights,
         prose subtitle height fix (0.70 for 2-line wrap)
 ============================================================ */
...
// ============================================================
// CLIENT: PROSE — basic narrative slide [v6.0.7, fixed v6.0.8]
...
// ============================================================
// CLIENT: TWOCOLS — 2-column image + text spotlights [v6.0.8]
// 16:9 image placeholders with subtitle + body per column.
// Image boxes are PPTX-editable: right-click → Picture Fill.
// ============================================================
...
function layoutTwoCols(cfg) {
  ...
  items.slice(0, cols).forEach(function(item, i) {
    ...
    els.push({
      type: 's', x: cx, y: startY, w: cw, h: imgH,
    ...
    els.push({
      type: 'p', text: 'IMAGE',
      x: cx + (cw - 1.00) / 2, y: startY + (imgH - 0.30) / 2,
      w: 1.00, h: 0.30,
      fill: isDark ? 'dkGray' : 'ltGray',
      color: isDark ? 'white' : 'title', size: 9
    });
    ...
    if (item.subtitle) {
      els.push({
        type: 't', text: item.subtitle,
        x: cx, y: textY, w: cw, h: 0.25,
    ...
    if (item.text) {
      els.push({
        ...
        x: cx, y: textY, w: cw, h: C.CONTENT_END - textY,
...
var LAYOUT_MAP = {
  ...
  fromto:layoutFromto, capability:layoutCapability, schedule:layoutSchedule,
  coverloc:layoutCoverloc, coverPresenter:layoutCoverPresenter,
  section:layoutSection, prose:layoutProse
};
